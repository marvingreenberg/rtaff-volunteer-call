...
| 3 | Don creates a new volunteer call ("Weekend Build — Demo …"). | |
...
| 6 | Vick Fisher logs in, checks every task, submits. | |
| 7 | Bryan Cobb logs in, checks the first two tasks, submits. | |
| 8 | Narration: "Overnight, 24 more volunteers responded…" `respond-availability --count 24` runs in parallel. | |
| 9 | Don logs back in, opens the Assignment Dashboard now full of responses, auto-assigns the first batch, clicks Done Assigning. | Status → `assigned`. |
